...
async function deleteSubscriptionByEmailAddress(client, pubId, emailAddress) {
  // note: this is a helper method that doesn't actually correspond directly to
  // specific api endpoint. the deletion endpoint that exists requires a
  // subscription id; so this method first makes a call to get the subscription
  // id that corresponds to the email address, and then calls the deletion
  ...
  return client.deleteSubscriptionById(pubId, response.json.data.id)
}

export { deleteSubscriptionByEmailAddress }
